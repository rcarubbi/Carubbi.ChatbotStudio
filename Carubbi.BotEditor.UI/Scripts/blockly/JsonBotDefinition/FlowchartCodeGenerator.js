...
Blockly.Flowchart.generalBlockToObj = function (block, workspace) {
    ...
    if (block && !block.isShadow_) {
        ...
        if (func) {
            ...
            console.log("Don't know how to generate Flowchart code for a '" + block.type + "'");
        }
    } else {
        return null;
    }
...
Blockly.Flowchart['botconfig'] = function (block, workspace) {
    ...
    var statements_steps = statementToArray(Blockly.Flowchart.statementToCode(block, 'Steps'));
    ...
    for (var i = 0; i < steps.length; i++) {
        flowchartConnections = flowchartConnections.concat(steps[i].flowchartConnections);
        flowchartDeclarations = flowchartDeclarations.concat(steps[i].flowchartDeclarations);
    }

    flowchartDeclarations.push(`root=>start: ${text_name}`);
    ...
    var code = flowchartDeclarations.map(x => `${x}:$onFlowchartNodeClicked`).reverse().join("\r\n") + "\r\n\r\n" + flowchartConnections.reverse().join("\r\n")
    return code;
};
...
Blockly.Flowchart['messagestep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: Mensagem ${label_id}|messagestep${selected}`);
...
Blockly.Flowchart['inputstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: Pergunta ${label_id}|inputstep${selected}`);
...
Blockly.Flowchart['confirmstep'] = function (block) {
    var label_id = block.getFieldValue("Id");
    var statements_truestep = statementToArray(Blockly.Flowchart.statementToCode(block, 'TrueStep'));
    var statements_falsestep = statementToArray(Blockly.Flowchart.statementToCode(block, 'FalseStep'));

    addStepsToLater(statements_truestep.concat(statements_falsestep));

    var trueStepId = (statements_truestep.length > 0 && statements_truestep[0].id) || null;
    var falseStepId = (statements_falsestep.length > 0 && statements_falsestep[0].id) || null;
    ...
    if (!trueStepId) {
        ...
        flowchartConnections.push(`step${label_id}(yes, left)->eYes${label_id}`);
    } else {
        flowchartConnections.push(`step${label_id}(yes, left)->step${trueStepId}`)
    }

    if (!falseStepId) {
        flowchartDeclarations.push(`eNo${label_id}=>end: Fim`);
        flowchartConnections.push(`step${label_id}(no, right)->eNo${label_id}`);
    } else {
        flowchartConnections.push(`step${label_id}(no, right)->step${falseStepId}`)
    }

    var selected = Blockly.Flowchart.SelectedId === label_id ? "selected" : "";

    flowchartDeclarations.push(`step${label_id}=>condition: Confirmação ${label_id}|confirmstep${selected}`);
...
Blockly.Flowchart['liststep'] = function (block) {
    var label_id = block.getFieldValue("Id");
    var listItems = statementToArray(Blockly.Flowchart.statementToCode(block, 'Input')).filter(x => x.targetStepId || x.action === "OpenURL" || x.action === "DownloadFile");
    ...
    for (var i = 0; i < listItems.length; i++) {
        if (i > 0) {
            flowchartDeclarations.push(`step${label_id}i${i + 1}=>parallel: Lista ${label_id} \r\n[${listItems[i].title}]|liststep${selected}`);
            if (i > 1) {
                flowchartConnections.push(`step${label_id}i${i}(path2, right)->step${label_id}i${i + 1}`);
                flowchartConnections.push(`step${label_id}i${i}@>step${label_id}i${i + 1}({"stroke": "${selected ? "yellow" : "#a5935b"}", "arrow-end": "none"})`);
            } else {
                flowchartConnections.push(`step${label_id}(path2, right)->step${label_id}i${i + 1}`);
                flowchartConnections.push(`step${label_id}@>step${label_id}i${i + 1}({"stroke": "${selected ? "yellow" : "#a5935b"}", "arrow-end": "none"})`);
            }
        }
        else {
            flowchartDeclarations.push(`step${label_id}=>parallel: Lista ${label_id} \r\n [${listItems[i].title}]|liststep${selected}`);
        }

        switch (listItems[i].action) {
            case "OpenURL":
            case "DownloadFile":
                var description = listItems[i].action === "OpenURL" ? `Abrir página` : `Baixar arquivo`;
                if (i > 0) {
                    flowchartDeclarations.push(`end${label_id}i${i + 1}=>end: ${description}`)
                    flowchartConnections.push(`step${label_id}i${i + 1}(path1, bottom)->end${label_id}i${i + 1}`);
                } else {
                    flowchartDeclarations.push(`end${label_id}=>end: ${description}`)
                    flowchartConnections.push(`step${label_id}(path1, bottom)->end${label_id}`);
                }
                break;
            default:
                if (i > 0) {
                    flowchartConnections.push(`step${label_id}i${i + 1}(path1, bottom)->step${listItems[i].targetStepId}`);
                } else {
                    flowchartConnections.push(`step${label_id}(path1, bottom)->step${listItems[i].targetStepId}`);
                }
                break;
        }
    }
...
Blockly.Flowchart['formstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: Questionário ${label_id}|formstep${selected}`);
...
Blockly.Flowchart['conditionstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>condition: Condição ${label_id}|conditionstep${selected}`);
...
Blockly.Flowchart['relationalexpression'] = function (block) {
    ...
    var value_left = parseValue(Blockly.Flowchart.valueToCode(block, 'Left', Blockly.Flowchart.ORDER_ATOMIC));
    var dropdown_operator = block.getFieldValue('Operator');
    var value_right = parseValue(Blockly.Flowchart.valueToCode(block, 'Right', Blockly.Flowchart.ORDER_ATOMIC));
...
Blockly.Flowchart['navigatelistitem'] = function (block) {
    ...
    var statements_targetstep = statementToArray(Blockly.Flowchart.statementToCode(block, 'TargetStep'));
...
Blockly.Flowchart['apistep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: API ${label_id}|apistep${selected}`);
...
Blockly.Flowchart['compositestep'] = function (block) {
    ...
    var steps = statements_steps;
    if (stepsToAddLater[_currentScope]) {
        steps = statements_steps.concat(stepsToAddLater[_currentScope].steps);
    }
    ...
    flowchartDeclarations.push(`step${label_id}=>subroutine: ${label_id} ${text_name}|compositestep${selected}`);
...
Blockly.Flowchart['faqstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: FAQ ${label_id}|faqstep${selected}`);
...
Blockly.Flowchart['mapsstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: Mapas ${label_id}|mapsstep${selected}`);
...
Blockly.Flowchart['switchstep'] = function (block) {
    ...
    var statements_cases = statementToArray(Blockly.Flowchart.statementToCode(block, 'Cases'));
    var label_id = block.getFieldValue("Id");
    ...
    for (var i = 0; i < statements_cases.length; i++) {
        if (i > 0) {
            flowchartDeclarations.push(`step${label_id}i${i + 1}=>parallel: Escolha ${text_input} ${label_id} \r\n Caso ${statements_cases[i].value}|switchstep${selected}`);
            if (i > 1) {
                flowchartConnections.push(`step${label_id}i${i}(path2, right)->step${label_id}i${i + 1}`);
                flowchartConnections.push(`step${label_id}i${i}@>step${label_id}i${i + 1}({"stroke": "${selected ? "yellow" : "#a5935b"}", "arrow-end": "none"})`);
                flowchartConnections.push(`step${label_id}i${i + 1}(path1, bottom)->step${statements_cases[i].targetStepId}`);
            } else {
                flowchartConnections.push(`step${label_id}(path2, right)->step${label_id}i${i + 1}`);
                flowchartConnections.push(`step${label_id}@>step${label_id}i${i + 1}({"stroke": "${selected ? "yellow" : "#a5935b"}", "arrow-end": "none"})`);
                flowchartConnections.push(`step${label_id}i${i + 1}(path1, bottom)->step${statements_cases[i].targetStepId}`);
            ...
            flowchartDeclarations.push(`step${label_id}=>parallel: Escolha ${text_input} ${label_id} \r\n Caso ${statements_cases[i].value}|switchstep${selected}`);
            flowchartConnections.push(`step${label_id}(path1, bottom)->step${statements_cases[i].targetStepId}`);
        }
    }
    ...
    if (nextBlock) {
        ...
        if (statements_cases.length === 0) {
            ...
            flowchartDeclarations.push(`step${label_id}i${statements_cases.length + 1}=>parallel: Escolha ${text_input} ${label_id} \r\n Outro caso|switchstep${selected}`);
            if (statements_cases.length > 1) {
                flowchartConnections.push(`step${label_id}i${statements_cases.length}(path2, right)->step${label_id}i${statements_cases.length + 1}`)
                flowchartConnections.push(`step${label_id}i${statements_cases.length}@>step${label_id}i${statements_cases.length + 1}({"stroke": "${selected ? "yellow" : "#a5935b"}"})`);
            } else {
                flowchartConnections.push(`step${label_id}(path2, right)->step${label_id}i${statements_cases.length + 1}`);
                flowchartConnections.push(`step${label_id}@>step${label_id}i${statements_cases.length + 1}({"stroke": "${selected ? "yellow" : "#a5935b"}"})`);
            }
            flowchartConnections.push(`step${label_id}i${statements_cases.length + 1}(path1, bottom)->step${nextBlockId}`)
        }
    } else if (statements_cases.length === 0) {
        flowchartDeclarations.push(`step${label_id}=>parallel: Escolha ${text_input} ${label_id}|switchstep${selected}`);
        flowchartDeclarations.push(`e${label_id}=>end: Fim`);
        flowchartConnections.push(`step${label_id}(path1)->e${label_id}`);
    }
...
Blockly.Flowchart['imageclassificationstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: Classificação de Imagem ${label_id}|imageclassificationstep${selected}`);
...
Blockly.Flowchart['readgpslocationstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: Leitura de localização ${label_id}|readgpslocationstep${selected}`);
...
Blockly.Flowchart['transformstep'] = function (block) {
    ...
    flowchartDeclarations.push(`step${label_id}=>operation: Transformação ${label_id}|transformstep${selected}`);
